...
const AWS = ({ setLoggedInUser }) => {
    ...
    const COGNITO_DOMAIN = 'ap-south-1iqxuzzcbn.auth.ap-south-1.amazoncognito.com';
    const CLIENT_ID = '6877ph5ofmlnpo8tetot0bguut';
    const REDIRECT_URI = 'https://d21a3j72wk3f7t.cloudfront.net/auth-callback';

    const getGoogleSignInUrl = () => {
        const responseType = 'code';
        const scope = encodeURIComponent('openid email profile');
        const identityProvider = 'Google';
        return `https://${COGNITO_DOMAIN}/oauth2/authorize?identity_provider=${identityProvider}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=${responseType}&client_id=${CLIENT_ID}&scope=${scope}`;
    };
    ...
    cognitoUser.confirmRegistration(signUpVerificationCode, true, (err) => {
        ...
        setTimeout(() => {
            navigate('/loginaws');
            window.location.reload(); // reloads after redirect
        }, 0);
    });
};
                    ...
                    {isLogin ? (!isForgotPassword && !isResetPassword ? (
                        <form onSubmit={handleLogin} className="bg-white shadow-md p-8 rounded-lg w-full max-w-sm">
                            ...
                            <button
                                ...
                                className="w-full bg-red-600 text-white font-bold py-2 rounded-md hover:bg-red-700 transition mt-4"
                            >
                                Sign in with Google
                            </button>
                        ...
                        <form onSubmit={handleSubmit} className="bg-white shadow-md p-8 rounded-lg w-full max-w-sm">
                            {!isCodeSent ? (
                                <>
                                    ...
                                    <input
                                        ...
                                        required
                                        className="w-full mb-2 px-4 py-2 border-b-2 border-gray-300 focus:border-black focus:outline-none"
                                    />
                            ...
                            <p className="mt-4 text-sm text-center">
                                ...
                                <span onClick={() => setIsLogin(true)} className="cursor-pointer font-bold text-yellow-600">
                                    Login
                                </span>
                            </p>
                        </form>
